...
export function genesisTransactions() {
...
export function indyDidFromNamespaceAndInitialKey(namespace, initialKey) {
...
export async function isWalletClosed(email, password) {
    try {
        await indy.closeWallet(await indy.openWallet({ id: email }, { key: password }));
        return
    } catch (error) {
        console.log(error)
        if (error.indyName === "WalletNotFoundError") {
            return
        }
        await sleep(1000)
        return isWalletClosed(email, password)
    }
}
